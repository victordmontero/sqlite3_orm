...
TableResult Database::SelectTable(char *tableName, ...)
{
    ...
    for (nextArg = *(++pargs); nextArg != NULL; nextArg = *(pargs++))
    {
        if (*pargs == NULL)
            break;
        nextArg = *pargs;
...
long Database::InsertIntoTable(const char *tableName, std::map<std::string, std::string> columnValues)
{
    ...
    for (std::map<std::string, std::string>::iterator it = columnValues.begin(); it != columnValues.end(); it++)
    {
...
long Database::UpdateTable(const char *tableName, const char *whereExpr,
...
long Database::DeleteRow(const char *tableName, std::string whereExpr)
{
    ...
    result += "DELETE FROM " + std::string(tableName);
